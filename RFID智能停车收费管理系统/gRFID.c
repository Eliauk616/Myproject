...
// 高频RFID卡卡号
unsigned int cardid;
char cardBuff[4];//保存卡号的数组
unsigned char passwd[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};//保存密码的数组
char loft[20];//保存读取到的信息
...
*/
  int PassWD(int fd, char AorB, char *uid, unsigned char *passwd,char block_num) {
    ...
    for (int i = 0; i < 18; ++i) {
      printf("%x ", cmd[i]);//打印出秘钥验证发送的指令
    }
    printf("\n");
    int ret = 0;
    ret = write(fd, cmd, 0x12);//发送
    if (ret == -1) {
    ...
    unsigned char req[6];
    memset(req, 0, sizeof(req));
    ret = read(fd, req, 6);//接受秘钥
    printf("passwd is: \n\t");
    for (int i = 0; i < 6; ++i) {
      printf("%x ", req[i]);//把接受的秘钥打印
    }
    printf("\n");
    if (ret < 0) {
